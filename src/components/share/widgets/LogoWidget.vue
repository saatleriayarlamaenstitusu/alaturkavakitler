...
// Alaturka Vakitler logosu. İki varyant:
//   wide → işaret + ALATURKA/VAKİTLER kelime markası
//   mark → yalnızca iki parçalı işaret
// Yollar yalnızca dolguludur (stroke boyası yok) — export kuralına uygun.
...
<template>
  <svg
    ...
    :style="{ color }"
    role="img"
    aria-label="Alaturka Vakitler"
  >
    <g transform="translate(-30.049 -474.358)" fill="currentColor">
      <path :d="MARK_A" />
      <path :d="MARK_B" transform="scale(-1)" />
      <template v-if="!isMark">
        <path :d="WORD_ALATURKA" />
        <path :d="WORD_VAKITLER" />
      </template>
    </g>
  </svg>
</template>
